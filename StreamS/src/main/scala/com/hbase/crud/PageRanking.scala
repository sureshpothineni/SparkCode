...
object PageRanking {
  def main(args: Array[String]) {
    ...
    val joinValues = joins.reduceByKey((x, y) => x + y).mapValues(v => 0.15 + 0.85*v)
    
    val joins1 = links.join(ranks).map { case (pageId, (links, rank)) => links.map(dest => (dest, rank/links.size)) }
   
    val test = links.map(x => (x._1.split(" ")))
  }
}  
